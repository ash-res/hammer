...
#' \itemize{
#'  \item \code{by} \cr
#'  Vector of outcome beta-coefficients for the genetic variants retained after clumping
#'  \item \code{sy} \cr
#'  Vector of outcome standard errors for the genetic variants retained after clumping
#'  \item \code{bx} \cr
#'  Matrix of exposure beta-coefficients for the genetic variants retained after clumping (the k-th column corresponds to the k-th exposure)
#'  \item \code{sx} \cr
#'  Matrix of exposure standard errors for the genetic variants retained after clumping (the k-th column corresponds to the k-th exposure)
#'  \item \code{ld} \cr
#'  matrix of genetic variant (LD) correlations for the genetic variants retained after clumping
#'  \item \code{clump.thres} \cr
#'  the R^2 clumping threshold used
#' }
#'
#'
...
clump <- function(by,sy,bx,sx,ld,clump.thres){
  ...
  while(i<=length(snps)){
    if(snps[i] %in% colnames(ld)){
      del <- which(as.vector((ld[which(colnames(ld)==snps[i]),-which(colnames(ld)==snps[i])]^2))>clump.thres)
  ...
  return(list("by"=by,"sy"=sy,"bx"=bx,"sx"=sx,"ld"=ld,"clump.thres"=clump.thres))
}
