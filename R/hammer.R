...
cat("\nHeterogeneity statistic :", decimals(Q(est),3)," ( p-value :", decimals(pchisq(Q(est), df = nrow(bx)-ncol(bx), lower.tail = F),3),")")
...
return(list("est"=est,"se"=se.mwi,"condF"=condF,"numberIVs"=J,"Q"=Q(est),"Q.pval"=pchisq(Q(est), df = nrow(bx)-ncol(bx), lower.tail = F)))
}
