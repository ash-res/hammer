...
#'  \item \code{SigY} \cr
#'  a J-vector of standard errors, where the j-th element corresponds to the standard error of the j-th instrument association with the outcome
#'  \item \code{SigX} \cr
#'  a J x K matrix of standard errors, the (j,k)-th element corresponds to the standard error of the j-th instrument association with exposure k
#' }
#'
#'
...
cat("\nHeterogeneity statistic :", decimals(Q.save,3)," ( p-value :", decimals(pchisq(Q.save, df = nrow(bx)-ncol(bx), lower.tail = F),3),")")
...
return(list("est"=est,"se"=se.mwi,"condF"=condF,"numberIVs"=J,"Q"=Q.save,"Q.pval"=pchisq(Q.save, df = nrow(bx)-ncol(bx), lower.tail = F),"GamX"=GamX,"GamY"=GamY,"SigX"=sqrt(t(sapply(1:J,function(j){diag(SigX[[j]])}))),"SigY"=sqrt(SigY)))
}
...
return(list("est"=gmm.star,"se"=se.star,"kappa"=kappa.est,"kappa.se"=kappa.se,"condF"=condF,"numberIVs"=J,"GamX"=GamX,"GamY"=GamY,"SigX"=sqrt(t(sapply(1:J,function(j){diag(SigX[[j]])}))),"SigY"=sqrt(SigY)))
}
}
